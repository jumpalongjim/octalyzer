...
WORKDIR /src
COPY ["Octalyze.csproj", "Octalyze/"]
RUN dotnet restore "Octalyze/Octalyze.csproj"

WORKDIR "/src/Octalyze"
COPY . .
RUN dotnet build "Octalyze.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Octalyze.csproj" \
...
Execute unit tests
```
    cd "Octalyze/test/Octalyze.Tests"
    dotnet test
```

Deploy application
```
    cd "Octalyze/src/Octalyze"
